...
from metrics import get_cindex, get_rm2, get_mae
from dataset import *
...
def val(model, criterion, dataloader, device):
    ...
    print(df_pred)
    df_pred.to_csv('data/pred_val.csv', index=False)
    ...
    epoch_mae = get_mae(label, pred)
    running_loss.reset()

    return epoch_loss, epoch_cindex, epoch_r2, epoch_mae, pred_list
...
def main():
    ...
    test_loss, test_cindex, test_r2, test_mae, pred_list = val(model, criterion, test_loader, device)
    msg = "test_loss:%.4f, test_cindex:%.4f, test_r2:%.4f, test_mae:%.4f" % (test_loss, test_cindex, test_r2, test_mae)
    print(msg)
